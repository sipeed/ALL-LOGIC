...
static void poll_usb_hotplug_fallback(void);
...
/* Close USB handle of active device without destroying session (for FW upgrade). */
SR_API int ds_close_actived_device(void)
{
	if (lib_ctx.actived_device_instance == NULL)
		return SR_ERR_CALL_STATUS;
	...
	sr_info("Temp-close actived device for upgrade/IAP.");
	close_device_instance(lib_ctx.actived_device_instance);
	return SR_OK;
}
...
/* Match by bus/address — libusb may allocate a new device* after re-enumeration. */
static int usb_dev_match(libusb_device *a, libusb_device *b)
{
	if (a == NULL || b == NULL)
		return 0;
	if (a == b)
		return 1;
	return (libusb_get_bus_number(a) == libusb_get_bus_number(b) &&
		libusb_get_device_address(a) == libusb_get_device_address(b));
}
...
SR_PRIV int sr_usb_device_is_exists(libusb_device *usb_dev)
{
	...
	for (l = lib_ctx.device_list; l; l = l->next)
	{
		...
		if (dev->dev_type != DEV_TYPE_USB)
			continue;
		if (usb_dev_match((libusb_device *)dev->handle, usb_dev))
		{
			bFind = 1;
			break;
		}
...
static int update_device_handle(struct libusb_device *old_dev, struct libusb_device *new_dev)
{
	GSList *l;
	struct sr_dev_inst *dev;
	struct sr_dev_inst *fallback = NULL;
	...
	for (l = lib_ctx.device_list; l; l = l->next)
	{
		...
		/* Match by handle (DSLogic/CH32/SLogic) */
		if (dev->handle == (ds_device_handle)old_dev ||
		    (usb_dev_info != NULL && usb_dev_info->usb_dev == old_dev))
		{
			fallback = dev;
			bFind = 1;
			break;
		}
#ifdef HAVE_CH32_DEVICE
		if (dev->driver && dev->driver->name &&
		    strcmp(dev->driver->name, "ch32-logic") == 0) {
			ch32_count++;
			ch32_fallback = dev;
		}
#endif
		...
		if (dev->driver && dev->driver->name &&
		    strcmp(dev->driver->name, "slogic-16u3") == 0) {
			slogic_count++;
			slogic_fallback = dev;
		}
#endif
	...
	if (!bFind) {
		struct libusb_device_descriptor ndes;
		uint16_t nvid = 0, npid = 0;

		if (libusb_get_device_descriptor(new_dev, &ndes) == 0) {
			nvid = ndes.idVendor;
			npid = ndes.idProduct;
		}
#ifdef HAVE_CH32_DEVICE
		if (ch32_count == 1 && ch32_fallback != NULL &&
		    nvid == 0x1A86 && (npid == 0x5537 || npid == 0x5538)) {
			sr_info("CH32 reconnect: fall back to sole ch32-logic instance.");
			fallback = ch32_fallback;
			bFind = 1;
		} else
#endif
#ifdef HAVE_SLOGIC_DEVICE
...
#endif
#ifdef HAVE_PXLOGIC_DEVICE
		if (pxlogic_count == 1 && pxlogic_fallback != NULL &&
		    ((nvid == 0x1A86 && npid == 0x5237) ||
		     (nvid == 0x16C0 && npid == 0x05DC))) {
			sr_info("PXLogic reconnect: fall back to sole pxlogic instance.");
			fallback = pxlogic_fallback;
			bFind = 1;
		} else
#endif
#ifdef HAVE_ATKLOGIC_DEVICE
	...
	if (bFind && fallback != NULL)
	{
		...
		if (usb_dev_info != NULL) {
			...
			/* CH32 previously had no conn — create one now */
			usb_dev_info = sr_usb_dev_inst_new(bus, address);
			if (usb_dev_info) {
				usb_dev_info->usb_dev = new_dev;
				dev->conn = usb_dev_info;
			}
		}
		dev->handle = (ds_device_handle)new_dev;

		/*
		 * CH32 keeps a private libusb_device* for open/claim.
		 * Must refresh it + USB speed (USB2/USB3 switch).
		 */
#ifdef HAVE_CH32_DEVICE
		if (dev->driver && dev->driver->name &&
		    strcmp(dev->driver->name, "ch32-logic") == 0) {
			extern void ch32_on_usb_reconnected(
				struct sr_dev_inst *sdi,
				struct libusb_device *new_dev);
			ch32_on_usb_reconnected(dev, new_dev);
		}
#endif
#ifdef HAVE_SLOGIC_DEVICE
		if (dev->driver && dev->driver->name &&
		    strcmp(dev->driver->name, "slogic-16u3") == 0) {
			extern void slogic16u3_on_usb_reconnected(
				struct sr_dev_inst *sdi,
				struct libusb_device *new_dev);
			slogic16u3_on_usb_reconnected(dev, new_dev);
		}
#endif
#ifdef HAVE_PXLOGIC_DEVICE
		if (dev->driver && dev->driver->name &&
		    strcmp(dev->driver->name, "pxlogic") == 0) {
			extern void pxlogic_on_usb_reconnected(
				struct sr_dev_inst *sdi,
				struct libusb_device *new_dev);
...
static void hotplug_event_listen_callback(struct libusb_context *ctx, struct libusb_device *dev, int event)
{
	...
	if (event == USB_EV_HOTPLUG_ATTACH)
	{
		...
		if (lib_ctx.is_waitting_reconnect)
		{
			...
			else
			{
				...
				{
					/*
					 * Reconnect match failed (e.g. CH32 stale handle after
					 * USB3→USB2 port change). Keep detach_device_handle so
					 * process_detach_event() can drop the ghost entry; then
					 * a full re-scan adds the live device.
					 */
					sr_err("Update reconnected device handle error! can't find the old.");
					lib_ctx.detach_event_flag = 1;
				}
			}
		}
		if (bDone == 0)
	...
	else if (event == USB_EV_HOTPLUG_DETTACH)
	{
		...
		if (lib_ctx.actived_device_instance != NULL 
			&& lib_ctx.actived_device_instance->dev_type == DEV_TYPE_USB
			&& usb_dev_match((libusb_device *)lib_ctx.actived_device_instance->handle, dev)
			&& ds_is_collecting())
		{
				...
				for (l = dev_list; l; l = l->next)
				{
					...
					for (l_check = lib_ctx.device_list; l_check; l_check = l_check->next)
					{
						...
						if (sdi->dev_type == DEV_TYPE_USB &&
						    ...
						    usb_dev_match((libusb_device *)sdi->handle,
								  (libusb_device *)new_sdi->handle)) {
							bFind = 1;
							break;
						}
					...
					if (!bFind){
						...
						/* Free duplicate from re-scan (prefer destroy if available). */
						if (new_sdi->driver && new_sdi->driver->dev_destroy)
							new_sdi->driver->dev_destroy(new_sdi);
						else
							sr_dev_inst_free(new_sdi);
					}					
				}
				pthread_mutex_unlock(&lib_ctx.mutext);
...
static void process_detach_event()
{
	...
	for (l = lib_ctx.device_list; l; l = l->next)
	{
		...
		if (dev->dev_type == DEV_TYPE_USB &&
		    usb_dev_match((libusb_device *)dev->handle, ev_dev))
		{
...
static gpointer usb_hotplug_process_proc(gpointer data)
{
	...
	while (!lib_ctx.lib_exit_flag)
	{
		...
		/*
		 * On Windows, libusb often reports HAS_HOTPLUG=0, so native
	...
	for (i = 0; i < num; i++){
		...
		for (l = lib_ctx.device_list; l; l = l->next){
			...
			if (dev_ins->dev_type != DEV_TYPE_USB)
				continue;
			if (usb_dev_match((libusb_device *)dev_ins->handle, array[i])){
				bFind = 1;
				break;
			}
		}
...
/**
 * Windows / backends without libusb hotplug: poll USB list and synthesize
 * attach/detach events so UI can prompt like the official DSView build.
 */
static void poll_usb_hotplug_fallback(void)
{
	struct libusb_device *dev;

	if (lib_ctx.sr_ctx == NULL || lib_ctx.sr_ctx->libusb_ctx == NULL)
		return;

	/* Skip if a previous event is still queued */
	if (lib_ctx.attach_event_flag || lib_ctx.detach_event_flag)
		return;

	/*
	 * Detach first while reconnect-wait is idle, so a quick unplug is seen
	 * before a new plug on another port is treated as pure attach.
	 */
	if (!lib_ctx.is_waitting_reconnect) {
		dev = get_new_detached_usb_device();
		if (dev != NULL) {
			sr_info("Poll hotplug: device detached handle=%p", dev);
			hotplug_event_listen_callback(lib_ctx.sr_ctx->libusb_ctx,
						      dev, USB_EV_HOTPLUG_DETTACH);
			return;
		}
	}
	...
	dev = get_new_attached_usb_device();
	if (dev != NULL) {
		sr_info("Poll hotplug: device attached handle=%p", dev);
		hotplug_event_listen_callback(lib_ctx.sr_ctx->libusb_ctx,
					      dev, USB_EV_HOTPLUG_ATTACH);
	}
}
